...
<head>
	...
	[#if productCategory??]
		[@seo type = "productList"]
			...
			[/#if]
			<title>[#if productCategory.resolveSeoTitle()?has_content]${productCategory.resolveSeoTitle()}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - Powered By IGOMALL[/#if]</title>
		[/@seo]
	[#else]
		<title>${message("shop.product.title")}[#if showPowered] - Powered By IGOMALL[/#if]</title>
	[/#if]
